...
const TraefikCertificateManagement = () => {
  ...
  const [isMainDomainMode, setIsMainDomainMode] = useState(false);
  // Main domain input
  ...
  const handleIssueCertificate = async () => {
    ...
    // For main domain mode
      ...
      try {
        ...
        // Request certificate for main domain through Traefik
        const response = await axios.post('/api/traefik-certificates/main-domain', {
          domain: mainDomain,
          applicationPort
        });
        
        toast.success('Certificate configuration initiated for main domain');
        ...
        console.error('Error configuring main domain certificate:', error);
        
        if (error.response?.data?.message) {
          toast.error(error.response.data.message);
        } else {
          toast.error('Failed to configure main domain certificate');
        }
      } finally {
        setIssuingCertificate(false);
      }
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-8">
        <h3 className="text-xl font-semibold mb-4">Configure New SSL Certificate with Traefik</h3>
        
        {/* Certificate type toggle */}
        <div className="mb-6">
          <div className="flex items-center space-x-4">
            <button
              className={`px-4 py-2 rounded-md ${!isMainDomainMode ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
              onClick={() => setIsMainDomainMode(false)}
            >
              Subdomain Certificate
            </button>
            <button
              className={`px-4 py-2 rounded-md ${isMainDomainMode ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
              onClick={() => setIsMainDomainMode(true)}
            >
        ...
        {/* Domain selection based on mode */}
        {!isMainDomainMode ? (
          // Subdomain selection
          <div className="mb-4">
          ...
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-500 mt-1">
              Enter your root domain that's registered in AWS Route53.
            </p>
          </div>
        )}
        ...
        <div className="mb-6">
          ...
          />
          <p className="text-sm text-gray-500 mt-1">
            The port your application is running on (default: 80)
          </p>
        </div>
        ...
        <button
          ...
          disabled={issuingCertificate || (!selectedSubdomain && !isMainDomainMode) || (!mainDomain && isMainDomainMode)}
        >
          {issuingCertificate ? 'Configuring Certificate...' : 'Configure Certificate'}
        </button>
      </div>
