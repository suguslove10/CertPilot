  ...
  const handleIssueCertificate = async () => {
    ...
    // For main domain mode
      ...
      try {
        ...
        const response = await axios.post('/api/certificates/main-domain', {
          domain: mainDomain,
          containerIds: selectedContainers.length ? selectedContainers : undefined
        });
        
        toast.success('Certificate issuance initiated for main domain');
        ...
        console.error('Error issuing main domain certificate:', error);
        
        if (error.response?.data?.message) {
          toast.error(error.response.data.message);
        } else {
          toast.error('Failed to issue main domain certificate');
        }
      } finally {
        setIssuingCertificate(false);
      }
