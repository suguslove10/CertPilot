...
/**
 * Generate router configuration for a main domain
 * @param {Object} domainConfig - Object containing domain configuration details
 * @returns {Promise<string>} - Path to the generated config file
 */
const generateMainDomainConfig = async (domainConfig) => {
  try {
    await ensureDynamicDir();
    
    const domain = domainConfig.parentDomain;
    const routerName = `main-${domain.replace(/\./g, '-')}`;
    
    // Use the user-specified port or default to 80
    const applicationPort = domainConfig.applicationPort || 80;
    
    console.log(`Generating configuration for main domain ${domain} with port ${applicationPort}`);
    
    // Create router configuration for main domain
    const config = {
      http: {
        routers: {
          [routerName]: {
        ...
        services: {
          [routerName]: {
            loadBalancer: {
              servers: [{ url: `http://${domainConfig.targetIp || 'localhost'}:${applicationPort}` }]
            }
          }
        },
        middlewares: {
    ...
    console.log(`Generated Traefik configuration for main domain ${domain} at ${configPath}`);
    
    return configPath;
  } catch (error) {
    console.error(`Error generating Traefik configuration for main domain: ${error.message}`);
    throw error;
  }
};
