...
// Add a demo route to create sample Traefik certificates
router.post('/create-samples', async (req, res) => {
  try {
    // Check if there are already certificates
    const existingFiles = await traefikManager.listCertificateFiles();
    
    if (existingFiles.length > 0) {
      return res.status(400).json({ message: 'Sample Traefik certificates already exist' });
    }
    
    // Create a sample Traefik configuration file
    const sampleConfig = `
# Traefik dynamic configuration for example.com
http:
  routers:
    example-web:
...
`;
    ...
    await traefikManager.writeDynamicConfig('example-com.yml', sampleConfig);
    
    // Create a sample cert info file
    const sampleCerts = [
      {
        domain: 'example.com',
        ...
        configFile: 'example-com.yml'
      },
      {
        domain: 'api.example.com',
        sans: [],
        expires: new Date(Date.now() + 85 * 24 * 60 * 60 * 1000), // 85 days
        provider: 'letsencrypt',
        configFile: 'api-example-com.yml'
      },
      {
        ...
        provider: 'letsencrypt',
        configFile: 'admin-example-com.yml'
      }
    ];
    
    // Save certificate info to a file or database
    const fs = require('fs').promises;
    const path = require('path');
    const certInfoFile = path.join(traefikManager.getDynamicConfigDir(), 'cert-info.json');
    await fs.writeFile(certInfoFile, JSON.stringify(sampleCerts, null, 2));
    
    res.status(201).json({ 
      message: 'Sample Traefik certificates created successfully',
      count: sampleCerts.length
    });
  } catch (error) {
    console.error('Error creating sample Traefik certificates:', error);
    res.status(500).json({ message: 'Error creating sample Traefik certificates' });
  }
});
