...
router.get('/count', protect, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error fetching certificate count' });
  }
});
...
router.get('/', protect, async (req, res) => {
  try {
    ...
    let certificates = await Certificate.find({ userId: req.user._id })
      .populate('subdomainId', 'name parentDomain')
      .sort({ createdAt: -1 });
    
    // For main domain certificates (where subdomainId is null but isMainDomain is true),
    // we need to ensure the frontend still gets proper data structure
    certificates = certificates.map(cert => {
      const certObj = cert.toObject();
      
      // If this is a main domain certificate, add special handling
      if (certObj.isMainDomain && !certObj.subdomainId) {
        // The domain itself has all the info we need
        certObj.mainDomain = true;
      }
...
// @route   POST /api/certificates/main-domain
// @desc    Issue a new SSL certificate for a main domain
// @access  Private
router.post('/main-domain', protect, async (req, res) => {
  const { domain, containerIds } = req.body;
  ...
  try {
    ...
    // Extract parent domain to verify it exists in Route53
    ...
    // Return success immediately without waiting for cert issuance
    res.status(201).json({ 
      message: 'Certificate request initiated for main domain',
      certificateId: certificate._id,
      domain
    });
    ...
    setTimeout(async () => {
      try {
        // Issue certificate
        console.log('Using certbot for main domain certificate issuance');
        const certResult = await issueCertificate(domain, req.user.email, req.user._id);
        ...
        // Handle container installation if provided
        if (containerIds && containerIds.length) {
          ...
          for (const containerId of containerIds) {
            try {
              ...
              // Update certificate based on installation result
              // Logic similar to subdomain certificate installation
              if (installResult.success) {
                certificate.status = 'installed';
                certificate.installDate = Date.now();
        ...
        console.log(`Certificate for main domain ${domain} issued successfully`);
      } catch (error) {
        console.error(`Background certificate issuance error: ${error.message}`);
        certificate.status = 'error';
        certificate.errorMessage = error.message;
...
// Add a demo route to create sample certificates
router.post('/create-samples', async (req, res) => {
  try {
    ...
    if (existingCount > 0) {
      return res.status(400).json({ message: 'Sample certificates already exist' });
    }
    ...
    const sampleCertificates = [
      ...
      },
      {
        ...
        issuer: 'Let\'s Encrypt',
        validFrom: new Date(),
        validTo: new Date(Date.now() + 80 * 24 * 60 * 60 * 1000), // 80 days
        status: 'active',
        type: 'domain-validated',
        provider: 'letsencrypt'
      },
    ...
    res.status(500).json({ message: 'Error creating sample certificates' });
  }
});
